...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    // 分别对入参做范围判断，然后赋值给loadFactor和threshold
    public HashMap(int initialCapacity, float loadFactor) {
    	...
    	// tableSizeFor(initialCapacity)对传入的容量取大于等于该值的最小的2的幂
        this.threshold = tableSizeFor(initialCapacity);
    }
    
    // 调用上面的构造方法，loadFactor赋值默认0.75
    public HashMap(int initialCapacity) {
        this(initialCapacity, DEFAULT_LOAD_FACTOR);
    }

    // 赋值loadFactor为默认的负载因子0.75
    public HashMap() {
        this.loadFactor = DEFAULT_LOAD_FACTOR; // all other fields defaulted
    }

    // loadFactor默认赋值0.75，把入参Map中的数据存入
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        // table未被初始化或者长度为0，先扩容，返回数组长度n
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;
        // 通过n-1&hash得到对应数组下标，若该位置没有元素，则直接插入新元素
        if ((p = tab[i = (n - 1) & hash]) == null)
            tab[i] = newNode(hash, key, value, null);
        // 否则该位置已经有元素了，我们就需要进行一些其他操作
        else {
            Node<K,V> e; K k;
            // 如果插入的key和原来的key相同，则替换
            if (p.hash == hash &&
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    // 
                        ...
                        do {
                            next = e.next;
                            /**
                         * newTab的容量是以前旧表容量的两倍,因为数组table下标是通过
                         * （table.length-1）& hash计算得到，因此扩容后，存放的位置就可能发生变化
